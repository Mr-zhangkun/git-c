...
void Hungry() {
	if (cgiFormCheckboxSingle("Hungry") == cgiFormSuccess) {
		fprintf(cgiOut, "chi<BR>\n");
	} else {
		fprintf(cgiOut, "buchi!<BR>\n");
	}
}
...
void Color() {
	int colorChoice;
	cgiFormSelectSingle("colors", colors, 4, &colorChoice, 0);
...
void RadioButtons() {
	...
	cgiFormRadio("age", ages, 5, &ageChoice, 0);
...
void ShowForm()
{
	...
	fprintf(cgiOut, "<option value=\"ABC\">ABC\n");
	...
	fprintf(cgiOut, "<input type=\"radio\" name=\"age\" value=\"h\">h\n");
